...
def bleep_pcm(
    ...
    *,
    ...
    restarts at each span's onset so it begins at zero.
    """
    out = list(samples)
    for start, end in spans:
        ...
        for i in range(i0, i1):
            t = (i - i0) / sample_rate
            out[i] = int(amplitude * math.sin(2 * math.pi * freq * t))
    return out
...
def _decode_pcm(src) -> Tuple[int, array]:
    ...
    decoded = str(Path(tempfile.mkdtemp()) / "decoded.wav")
    subprocess.run(
...
def _write_wav(out_path, sample_rate: int, samples: Sequence[int]) -> Path:
    clamped = array("h", (max(INT16_MIN, min(INT16_MAX, s)) for s in samples))
