...
def test_inset_shrinks_span_inward_on_each_edge():
    [out] = postprocess_spans([span(1.0, 1.5)], inset=0.1)
...
def test_span_that_collapses_to_zero_or_less_is_dropped():
    ...
    assert postprocess_spans([span(1.0, 1.04)], inset=0.03) == []


def test_overlapping_spans_merge_into_one_covering_both():
    out = postprocess_spans([span(1.0, 2.0), span(1.5, 2.5)], inset=0.0)
    assert len(out) == 1
...
def test_non_overlapping_spans_are_kept_separate():
    out = postprocess_spans([span(1.0, 2.0), span(3.0, 4.0)], inset=0.0)
    assert len(out) == 2
...
def test_merged_span_joins_both_words_and_takes_min_confidence():
    ...
    [out] = postprocess_spans([a, b], inset=0.0)
