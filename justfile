...
# A two-line wrapper rather than a symlink or a copy. It execs this repo's own
# entry point, so there is still one environment and edits here take effect at
# once — but it also carries this shell's ffmpeg across in $PODIO_FFMPEG.
# That matters because an episode directory is outside the dev shell, and the
# ffmpeg it finds there is whatever the machine happens to have: podio parses
# ffmpeg's human-readable output, and records through the OpenAL device that
# the default builds leave out. Re-run this after `nix flake update`, when the
# store path it recorded moves.
install bindir="~/.local/bin": sync
    #!/usr/bin/env bash
    set -euo pipefail
    dir="$(eval echo {{bindir}})"
    mkdir -p "$dir"
    entry="$(pwd)/.venv/bin/podio"
    ffmpeg_path="$(command -v ffmpeg)"
    rm -f "$dir/podio"  # an older install left a symlink; writing would follow it
    printf '#!/usr/bin/env bash\n# generated by `just install` — re-run it after `nix flake update`\nexport PODIO_FFMPEG="${PODIO_FFMPEG:-%s}"\nexec "%s" "$@"\n' "$ffmpeg_path" "$entry" > "$dir/podio"
    chmod +x "$dir/podio"
    echo "installed $dir/podio -> $entry"
...
bumper out="bumper.wav" device="":
    uv run podio bumper "{{out}}" {{ if device != "" { "--device " + device } else { "" } }}
