...
manifest audio out="manifest.json" model="base.en" wordlist="config/wordlist.yaml":
    PYTHONPATH=src .venv/bin/python -m bleep.cli manifest {{audio}} --out {{out}} --wordlist {{wordlist}} --model {{model}}
